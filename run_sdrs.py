...
sdrs.set_fir_coeffs(100, 250 * 10**3)
processing.DOA_inter_elem_space = 0.1766
processing.update_data()
...
def draw_graphs(i):
    ...
    [delay.plot(processing.delay_log[i]) for i in range(0, 3)]
    [phase.plot(processing.phase_log[i]) for i in range(0, 3)]
    ...
    doa.plot(range(-90, 91), [np.mean(k) for k in zip(*[np.array(x) for x in doa_avg])])
